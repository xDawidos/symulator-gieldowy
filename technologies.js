...
const technologies = {
    ...
    'COMMON_MANAGEMENT_1': { name: "Szkolenie Agile", description: "Wdrożenie zwinnych metodyk zarządzania projektami poprawia ogólną kondycję finansową firmy.", type: "wzmacnianie rozwoju", cost: 120, initialCashCost: 1200, sector: 'common', icon: '🧑‍🏫', applyEffect: (stock) => { stock.financialHealth += 0.5; logEvent(`🎓 Wdrożenie Agile w ${stock.name} poprawia jej kondycję.`); } },
    ...
    'COMMON_FINANCE_1': { name: "Zaawansowany Audyt Finansowy", description: "Przeprowadzenie dogłębnego audytu pozwala zoptymalizować koszty i zwiększyć rentowność.", type: "wzmacnianie ceny i zysków", cost: 220, initialCashCost: 2200, sector: 'common', icon: '💰', applyEffect: (stock) => { stock.financialHealth += 0.5; stock.volatilityFactor *= 0.97; } },
    ...
    'COMMON_LEGAL_1': { name: "Optymalizacja Podatkowa", description: "Nowa strategia podatkowa pozwala na legalne obniżenie obciążeń fiskalnych, co zwiększa zysk netto.", type: "wzmacnianie ceny i zysków", cost: 400, initialCashCost: 4000, sector: 'common', icon: '🧾', applyEffect: (stock) => { stock.financialHealth += 1.5; } },
    ...
    'COMMON_EXPANSION_1': { name: "Badanie Rynków Zagranicznych", description: "Analiza potencjału eksportowego otwiera drzwi do przyszłej ekspansji i nowych źródeł przychodu.", type: "wzmacnianie rozwoju", cost: 250, initialCashCost: 2500, sector: 'common', icon: '🌍', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.05, 'positive'); } },
    ...
    'INDUSTRY_PLASTAL_1': { name: "Plastal", icon: '🧱', description: "Opatentowanie nowego, ultralekkiego kompozytu otwiera rynek premium.", type: "wzmacnianie ceny i zysków", cost: 300, initialCashCost: 3000, sector: 'Przemysł', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.08, 'positive'); } },
    ...
    'INDUSTRY_ROBO_1': { name: "Roboinżynieria", icon: '🦾', description: "Automatyzacja linii produkcyjnej zwiększa wydajność i jakość.", type: "wzmacnianie ceny i zysków", cost: 400, initialCashCost: 4000, sector: 'Przemysł', applyEffect: (stock) => { stock.financialHealth += 1.2; stock.volatilityFactor *= 0.95; } },
    ...
    'INDUSTRY_QUANTUM_BARRACKS_1': { name: "Baraki Kwantowe", icon: '🌌', description: "Zastosowanie fizyki kwantowej do... budowy baraków? Brzmi innowacyjnie.", type: "wzmacnianie pozycji na rynku", cost: 500, initialCashCost: 5000, sector: 'Przemysł', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.10, 'positive'); } },
    ...
    'BANKING_SECURITY_2': { name: "Sejfy w Innym Wymiarze", icon: '🌀', description: "Przełomowa, kwantowa technologia zabezpieczeń staje się legendą na rynku.", type: "wzmacnianie pozycji na rynku", cost: 500, initialCashCost: 5000, sector: 'Bankowość', applyEffect: (stock) => { stock.volatilityFactor *= 0.8; stock.financialHealth += 1.0; } },
    ...
    'BANKING_SONIC_1': { name: "Bankowość Elektrosoniczna", icon: '🔊', description: "Autoryzacja transakcji za pomocą unikalnego tonu głosu klienta.", type: "wzmacnianie pozycji na rynku", cost: 320, initialCashCost: 3200, sector: 'Bankowość', applyEffect: (stock) => { stock.financialHealth += 0.9; } },
    'BANKING_CRASH_AI_1': { name: "AI Przewidujące Krachy", icon: '🔮', description: "System wczesnego ostrzegania przed kryzysami, co znacząco stabilizuje portfel banku.", type: "wzmacnianie rozwoju", cost: 600, initialCashCost: 6000, sector: 'Bankowość', applyEffect: (stock) => { stock.volatilityFactor *= 0.75; } },
    ...
    'FOOD_VITAMIN_1': { name: "Sok Multiwitamina", icon: '🧃', description: "Wprowadzenie na rynek nowego, prozdrowotnego produktu staje się hitem sprzedażowym.", type: "wzmacnianie pozycji na rynku", cost: 220, initialCashCost: 2200, sector: 'Żywność', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.06, 'positive'); } },
    'FOOD_NO_ROT_1': { name: "Niegnijąca Mąka", icon: '🍞', description: "Opatentowanie metody przechowywania, która rewolucjonizuje logistykę i zapasy.", type: "wzmacnianie rozwoju", cost: 280, initialCashCost: 2800, sector: 'Żywność', applyEffect: (stock) => { stock.financialHealth += 1.2; } },
    'FOOD_EDIBLE_CUP_1': { name: "Jadalne Kubki", icon: '☕', description: "Innowacyjny, ekologiczny produkt, który staje się viralem w mediach.", type: "wzmacnianie pozycji na rynku", cost: 190, initialCashCost: 1900, sector: 'Żywność', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.05, 'positive'); } },
    ...
    'GAMING_ADDICTION_1': { name: "System Uzależniania Graczy", icon: '🎰', description: "Kontrowersyjny, ale skuteczny system pętli behawioralnych maksymalizuje przychody.", type: "wzmacnianie ceny i zysków", cost: 400, initialCashCost: 4000, sector: 'Gaming', applyEffect: (stock) => { stock.financialHealth += 2.0; stock.volatilityFactor *= 1.1; } },
    'GAMING_SLEEP_1': { name: "Granie Przez Sen", icon: '😴', description: "Technologia pozwalająca na zdobywanie postępów w grze podczas snu. Gracze to kochają.", type: "wzmacnianie pozycji na rynku", cost: 600, initialCashCost: 6000, sector: 'Gaming', applyEffect: (stock) => { stock.financialHealth += 1.5; } },
    ...
    'TECH_QUANTUM_1': { name: "Komputery Kwantowe", icon: '⚛️', description: "Przełom w obliczeniach kwantowych daje firmie ogromną przewagę technologiczną.", type: "wzmacnianie pozycji na rynku", cost: 1500, initialCashCost: 15000, sector: 'Technologia', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.30, 'positive'); } },
    'TECH_HOLO_1': { name: "Holograficzne Telekonferencje", icon: '👨‍🚀', description: "System do telekonferencji, który wyświetla rozmówców jako trójwymiarowe hologramy.", type: "wzmacnianie pozycji na rynku", cost: 600, initialCashCost: 6000, sector: 'Technologia', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.12, 'positive'); } },
    ...
    'REALTY_3DPRINT_1': { name: "Domy z Drukarki 3D", icon: '🖨️', description: "Zastosowanie druku 3D do wznoszenia budynków rewolucjonizuje rynek budowlany.", type: "wzmacnianie rozwoju", cost: 600, initialCashCost: 6000, sector: 'Nieruchomości', applyEffect: (stock) => { stock.financialHealth += 2.0; stock.volatilityFactor *= 0.85; } },
    ...
    'REALTY_ANTIGRAV_1': { name: "Antygrawitacyjne Fundamenty", icon: '🚀', description: "Pozwalają budować na dowolnym terenie. Fizycy twierdzą, że to niemożliwe, ale działa.", type: "wzmacnianie pozycji na rynku", cost: 1500, initialCashCost: 15000, sector: 'Nieruchomości', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.35, 'positive'); } },
    ...
    'REALTY_AI_ARCHITECT_1': { name: "Architekt AI", icon: '🏛️', description: "AI, która projektuje zoptymalizowane i piękne budynki w ułamku sekundy.", type: "wzmacnianie rozwoju", cost: 450, initialCashCost: 4500, sector: 'Nieruchomości', applyEffect: (stock) => { stock.financialHealth += 1.8; } },
    ...
    // Energia
    'ENERGY_FUSION_1': { name: "Reaktor Fuzyjny", icon: '☀️', description: "Przełom w badaniach nad fuzją jądrową obiecuje tanią, czystą energię.", type: "wzmacnianie pozycji na rynku", cost: 2000, initialCashCost: 20000, sector: 'Energia', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.50, 'positive'); stock.financialHealth += 2; } },
    ...
    'ENERGY_BLACK_HOLE_1': { name: "Energia z Osobliwości", icon: '⚫', description: "Niezwykle wydajne, choć nieco niepokojące źródło energii.", type: "wzmacnianie ceny i zysków", cost: 3000, initialCashCost: 30000, sector: 'Energia', applyEffect: (stock) => { stock.financialHealth += 10; } },
    ...
    'ENERGY_KINETIC_1': { name: "Płytki Kinetyczne", icon: '🚶', description: "Płytki chodnikowe, które generują energię z kroków przechodniów.", type: "wzmacnianie pozycji na rynku", cost: 300, initialCashCost: 3000, sector: 'Energia', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.05, 'positive'); } },
    'ENERGY_SOLAR_PAINT_1': { name: "Farba Fotowoltaiczna", icon: '🎨', description: "Farba, którą można pomalować dowolną powierzchnię, zamieniając ją w panel słoneczny.", type: "wzmacnianie ceny i zysków", cost: 600, initialCashCost: 6000, sector: 'Energia', applyEffect: (stock) => { stock.financialHealth += 1.5; applyPriceEffect(stock.symbol, 0.10, 'positive'); } },
    ...
    'CHEM_WATER_FUEL_1': { name: "Paliwo z Wody", icon: '💧', description: "Opatentowanie wydajnego katalizatora do elektrolizy wody.", type: "wzmacnianie rozwoju", cost: 1200, initialCashCost: 12000, sector: 'Chemia', applyEffect: (stock) => { stock.financialHealth += 4.0; } },
    ...
    'CHEM_AERO_GEL_1': { name: "Produkcja Aerożelu", icon: '🧊', description: "Uruchomienie produkcji najlżejszego materiału stałego na świecie o niezwykłych właściwościach izolacyjnych.", type: "wzmacnianie ceny i zysków", cost: 400, initialCashCost: 4000, sector: 'Chemia', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.10, 'positive'); } },
    'CHEM_MED_POLYMER_1': { name: "Polimery Medyczne", icon: '❤️', description: "Stworzenie biokompatybilnych polimerów do produkcji implantów i protez.", type: "wzmacnianie pozycji na rynku", cost: 450, initialCashCost: 4500, sector: 'Chemia', applyEffect: (stock) => { stock.financialHealth += 1.2; } },

    // Dobra konsumpcyjne
    'CONSUMER_3DPRINT_1': { name: "Druk 3D na Życzenie", icon: '🖨️', description: "Możliwość personalizacji produktów za pomocą druku 3D przyciąga klientów premium.", type: "wzmacnianie pozycji na rynku", cost: 280, initialCashCost: 2800, sector: 'Dobra konsumpcyjne', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.06, 'positive'); } },
    ...
    'CONSUMER_LOYALTY_AI_1': { name: "Program Lojalnościowy AI", icon: '❤️', description: "AI, które personalizuje zniżki i nagrody, maksymalizując lojalność klientów.", type: "wzmacnianie rozwoju", cost: 380, initialCashCost: 3800, sector: 'Dobra konsumpcyjne', applyEffect: (stock) => { stock.financialHealth += 1.2; } },
    ...
    'SERVICES_CLOUD_PLATFORM_1': { name: "Własna Platforma Chmurowa", icon: '☁️', description: "Stworzenie własnej infrastruktury chmurowej uniezależnia od gigantów i staje się nowym źródłem przychodu.", type: "wzmacnianie rozwoju", cost: 800, initialCashCost: 8000, sector: 'Usługi', applyEffect: (stock) => { stock.financialHealth += 2.5; } },
    'SERVICES_SUBSCRIPTION_1': { name: "Model Subskrypcyjny", icon: '🔁', description: "Przejście z jednorazowych usług na model subskrypcyjny, zapewniający stały dopływ gotówki.", type: "wzmacnianie ceny i zysków", cost: 300, initialCashCost: 3000, sector: 'Usługi', applyEffect: (stock) => { stock.volatilityFactor *= 0.9; } },
    'SERVICES_ON_DEMAND_1': { name: "Eksperci na Żądanie", icon: '💡', description: "Platforma łącząca firmy z wysoko wykwalifikowanymi ekspertami na godziny.", type: "wzmacnianie pozycji na rynku", cost: 350, initialCashCost: 3500, sector: 'Usługi', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.07, 'positive'); } },

    // Turystyka
    'TOURISM_VR_1': { name: "Wirtualne Wycieczki", icon: '🌐', description: "Oferowanie wycieczek w wirtualnej rzeczywistości jako nowa gałąź przychodów.", type: "wzmacnianie ceny i zysków", cost: 320, initialCashCost: 3200, sector: 'Turystyka', applyEffect: (stock) => { stock.financialHealth += 1.0; } },
    'TOURISM_SPACE_1': { name: "Turystyka Kosmiczna", icon: '🚀', description: "Organizacja lotów suborbitalnych dla najbogatszych klientów.", type: "wzmacnianie pozycji na rynku", cost: 2500, initialCashCost: 25000, sector: 'Turystyka', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.70, 'positive'); } },
    ...
    'TOURISM_TRANSLATOR_1': { name: "Tłumacz Neuronowy", icon: '🗣️', description: "Urządzenie, które w czasie rzeczywistym tłumaczy rozmowy na dowolny język.", type: "wzmacnianie rozwoju", cost: 600, initialCashCost: 6000, sector: 'Turystyka', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.10, 'positive'); } },
    'TOURISM_TIME_TRAVEL_1': { name: "Podróże w Czasie (Symulacje)", icon: '⏳', description: "Historyczne symulacje VR pozwalające 'odwiedzić' starożytny Rzym lub pole bitwy pod Grunwaldem.", type: "wzmacnianie pozycji na rynku", cost: 800, initialCashCost: 8000, sector: 'Turystyka', applyEffect: (stock) => { stock.financialHealth += 1.2; applyPriceEffect(stock.symbol, 0.15, 'positive'); } },
    ...
    // Medycyna
    'MED_GENE_1': { name: "Terapie Genowe", icon: '🧬', description: "Przełom w personalizowanych terapiach genowych daje nadzieję milionom pacjentów.", type: "wzmacnianie pozycji na rynku", cost: 1800, initialCashCost: 18000, sector: 'Medycyna', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 0.40, 'positive'); } },
    ...
    'MED_NANOBOTS_1': { name: "Nanoboty Naprawcze", icon: '🤖', description: "Mikroskopijne roboty krążące w krwiobiegu, które naprawiają uszkodzone komórki.", type: "wzmacnianie cen i zysków", cost: 2200, initialCashCost: 22000, sector: 'Medycyna', applyEffect: (stock) => { stock.financialHealth += 5.0; } },
    ...
    'MED_ANTI_AGING_1': { name: "Odwracanie Starzenia", icon: '👶', description: "Terapia, która cofa zegar biologiczny. Skutki uboczne są... nieznane.", type: "wzmacnianie pozycji na rynku", cost: 3000, initialCashCost: 30000, sector: 'Medycyna', applyEffect: (stock) => { applyPriceEffect(stock.symbol, 1.0, 'positive'); } },
